...
- **[gradient.rs](src/polygenvo/gradient.rs)** — on-device differentiable-rasterizer polish. `PolishCfg { enabled, every_k, steps_n, lr, tau_start, tau_end }` (default: disabled) and `PolishState` own the cached GPU pipelines+buffers; `PolishState::polish(&mut genome, parent_fitness, calc, cfg)` runs `steps_n` backward+Adam passes ([softraster.wgsl](src/polygenvo/softraster.wgsl) + [adam.wgsl](src/polygenvo/adam.wgsl)), then keeps the result only if the hard ΔE2000 renderer confirms improvement (elitist gate). The test-only [softras_ref.rs](src/polygenvo/softras_ref.rs) is the CPU reference oracle used to verify the GPU forward/backward passes.
