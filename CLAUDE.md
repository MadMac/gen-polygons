...
- `cargo run --release --bin polygenvo -- --gradient-polish` — every `PolishCfg.every_k`
  ...
  Adam in `gradient.rs`/`softraster_tiled.wgsl`/`adam.wgsl`) of vertex positions+colors,
  kept only if the hard ΔE2000 renderer confirms it beats the parent (preserving the
  (1+λ) no-regression guarantee). Default off; composable with `--infinite`/`--show-window`.
...
- **[fitness.rs](src/polygenvo/fitness.rs)** — the GPU evaluator. `FitnessCalc::fitness_of_batch(&[&[Vertex]]) -> Vec<Eval>` renders each candidate into an offscreen `Rgba8UnormSrgb` target (real OVER alpha blending; MSAA only on the finest level), then dispatches [fitness.wgsl](src/polygenvo/fitness.wgsl) to score it and emit a 16×16 residual-error grid. The compute shader converts rendered+goal pixels linear-RGB → CIE XYZ → CIELAB, takes per-pixel ΔE2000, and `atomicAdd`s a workgroup-reduced sum (scaled by `FITNESS_SCALE`) into one `u32`. Rust maps the accumulator to a similarity score in `[0, 1_000_000]`, **higher = better**. `FitnessCalc` is `Clone`-by-`Arc`. GPU-layout constants (`LAMBDA`, `FITNESS_SCALE`, `ERROR_GRID_DIM`/`GRID_CELLS`, slot strides, `MSAA_SAMPLE_COUNT`) and `build_pyramid` live here.
...
- **[gradient.rs](src/polygenvo/gradient.rs)** — on-device differentiable-rasterizer polish. `PolishCfg { enabled, every_k, steps_n, lr, tau_start, tau_end }` (default: disabled) and `PolishState` own the cached GPU pipelines+buffers; `PolishState::polish(&mut genome, parent_fitness, calc, cfg)` runs `steps_n` tiled backward+Adam passes ([softraster_tiled.wgsl](src/polygenvo/softraster_tiled.wgsl) + [adam.wgsl](src/polygenvo/adam.wgsl), with per-tile triangle binning by [binning.wgsl](src/polygenvo/binning.wgsl)), then keeps the result only if the hard ΔE2000 renderer confirms improvement (elitist gate). The GPU forward/backward/binning passes are cross-checked against the CPU reference oracle [softras_ref.rs](src/polygenvo/softras_ref.rs) by the harnesses in [gradient_tests.rs](src/polygenvo/gradient_tests.rs) — a `#[path]`-routed `#[cfg(test)]` child module, so `gradient.rs` itself stays production-only.
- **[es.rs](src/polygenvo/es.rs)** — the search driver. `pub(crate) fn run_es(device, queue, goal, cfg: EsConfig) -> EsResult` runs a **(1+λ)-ES** over the coarse-to-fine `PHASES` schedule: each step generates `cfg.lambda` candidates, scores them in one batch, accepts the best improver. Two extracted structs own the fiddly state — **`OneFifthRule`** (current σ pair + the 1/5-success-rule window, with a single `reset_window` so the per-type tallies can't drift) and **`PhaseSchedule`** (phase index, in-phase step count, plateau detection via `check_plateau`). On plateau, promotion raises the cap and re-scores at a finer pyramid level; the genome grows organically via `split` (no batch growth). `EsConfig` carries a `polish: PolishCfg` field; when enabled, `run_es` calls `PolishState::polish` every `every_k` accepted improvements. ES tunables (`SIGMA_*`, `SIGMA_WINDOW`, `PHASE_MIN_STEPS`, `PLATEAU_WINDOW`, `PLATEAU_ACCEPTS`, `MAX_STEPS`, `MIN_TRIANGLES`) live here.
...
Tunable constants now live next to the module that owns them (genome capacity in `genome.rs`, GPU/grid layout in `fitness.rs`, ES schedule/σ in `es.rs`) rather than one block at the top of `main.rs`. Tests are a `#[cfg(test)] mod tests` in each module (with shared fixtures in [test_support.rs](src/polygenvo/test_support.rs)); the bulkier GPU test scaffolding lives in dedicated `#[cfg(test)]` files — the CPU oracle [softras_ref.rs](src/polygenvo/softras_ref.rs) and the GPU-oracle harnesses [gradient_tests.rs](src/polygenvo/gradient_tests.rs).

## Shader pipeline

All shaders are loaded via `include_str!` and fed to `wgpu` as `ShaderSource::Wgsl` (two of them — `fitness.wgsl` and `softraster_tiled.wgsl` — are first prefixed with the shared `color.wgsl` prelude via `gpu::with_color_prelude`):
...
- [softraster_tiled.wgsl](src/polygenvo/softraster_tiled.wgsl) — the differentiable soft-rasterizer (`forward`/`backward` compute passes, workgroup-per-tile) for `--gradient-polish`; reverse-transmittance O(num_tris) backward with CAS atomic-float gradient scatter.
- [binning.wgsl](src/polygenvo/binning.wgsl) — prefix-sum tile binning (count/scan/fill/sort) that builds the per-tile triangle lists `softraster_tiled.wgsl` iterates.
- [color.wgsl](src/polygenvo/color.wgsl) — canonical CIE `linear_rgb_to_xyz`/`xyz_to_lab`, prepended to `fitness.wgsl` and `softraster_tiled.wgsl` at build time so the fitness gate and the polish optimiser share one definition of Lab.
